    ...
    while(1):
        ...
        try:
            while True:
                ...
                if js1 or js2 != 0.00:
                    # for now only sending movment data
                    data_to_send = f"J{js1:.2f},{js2:.2f}\n"
                    # Send that data through serial
                ...
                # Triangle button to turn on and off the motors
                triangle_button = joystick.get_button(0)
                if triangle_button and not prev_triangle_button:
                    ser.write("M".encode())
                    print("M")
                
                prev_triangle_button = triangle_button

                # Circle button to reset the gyro to 0
                circle_button = joystick.get_button(1)
                if circle_button and not prev_circle_button:
                    ser.write("c".encode())
                    print("c")
                
                prev_circle_button = circle_button
                ...
                if distance < THRESHOLD_DISTANCE and mei_angry == False:
                    angry()
                    mei_angry = True
